 ...
 */
	...
	$.jstree.core.prototype = {
					...
					var dat = data.dat,
						...
						parse_flat = function (d, p, ps) {
							...
							for(i = 0, j = tmp.children.length; i < j; i++) {
							    //SK
							    var mAr = m[tmp.children[i]];
							    if (!mAr) mAr = tmp.children[i];
							    c = parse_flat(mAr, tmp.id, ps);
							    //SK c = parse_flat(m[tmp.children[i]], tmp.id, ps);
							    e = m[c];
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			for (i = 0, j = obj.children.length; i < j; i++) {
                //SK
			    var mAr = m[obj.children[i]];
			    if (!mAr) mAr = obj.children[i];
			    if (!mAr.state.hidden) {
			    //SK if(!m[obj.children[i]].state.hidden) {
					has_children = true;
					break;
				}
			}
			...
			{
				if(obj.state.loaded && !has_children) {
					node.childNodes[1].childNodes[0].className = 'glyphicon glyphicon-leaf';
					node.childNodes[1].childNodes[0].style.marginRight = "5px";
				}
			}
