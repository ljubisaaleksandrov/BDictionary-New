...
IF EXISTS (select top 1 1 from [dbo].[QuestionAnswer])
...
IF EXISTS (select top 1 1 from [dbo].[QuestionCategory])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
GO
PRINT N'Dropping [dbo].[FK_QuestionAnswer_QuestionCategory]...';


GO
ALTER TABLE [dbo].[QuestionAnswer] DROP CONSTRAINT [FK_QuestionAnswer_QuestionCategory];


GO
PRINT N'Dropping [dbo].[FK_QuestionCategory_ParentQuestionCategory]...';


GO
ALTER TABLE [dbo].[QuestionCategory] DROP CONSTRAINT [FK_QuestionCategory_ParentQuestionCategory];


GO
PRINT N'Dropping [dbo].[FK_QuestionAnswer_QuestionAnswerValueType]...';


GO
ALTER TABLE [dbo].[QuestionAnswer] DROP CONSTRAINT [FK_QuestionAnswer_QuestionAnswerValueType];


GO
PRINT N'Altering [dbo].[QuestionAnswer]...';


GO
ALTER TABLE [dbo].[QuestionAnswer] DROP COLUMN [QuestionAnswerValueTypeID];


GO
PRINT N'Starting rebuilding table [dbo].[QuestionCategory]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_QuestionCategory] (
    [Id]         INT            IDENTITY (1, 1) NOT NULL,
    [Name]       NVARCHAR (256) NOT NULL,
    [AnswerType] INT            NOT NULL,
    [ParentID]   INT            NULL,
    CONSTRAINT [tmp_ms_xx_constraint_PK_dbo.QuestionCategory] PRIMARY KEY CLUSTERED ([Id] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[QuestionCategory])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_QuestionCategory] ON;
        INSERT INTO [dbo].[tmp_ms_xx_QuestionCategory] ([Id], [Name], [ParentID])
        SELECT   [Id],
                 [Name],
                 [ParentID]
        FROM     [dbo].[QuestionCategory]
        ORDER BY [Id] ASC;
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_QuestionCategory] OFF;
    END

DROP TABLE [dbo].[QuestionCategory];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_QuestionCategory]', N'QuestionCategory';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_dbo.QuestionCategory]', N'PK_dbo.QuestionCategory', N'OBJECT';
...
GO
ALTER TABLE [dbo].[QuestionCategory] WITH NOCHECK
...
ALTER TABLE [dbo].[QuestionCategory] WITH CHECK CHECK CONSTRAINT [FK_QuestionCategory_ParentQuestionCategory];
